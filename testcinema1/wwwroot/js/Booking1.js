...
function updateTicketTable() {
    ...
    selectedSeats.forEach(function (seatId) {
        ...
        var seatRow = '<tr><td class="td-first yseat">' + seatInfo + '</td><td class="td-second align-right">x 65,000 VND</td></tr>';
...
function updateTable() {
    ...
    //totalMoney = 0; // Reset totalMoney before recalculating

    // Iterate over each combo
    $('.combo').each(function () {
        var comboName = $(this).find('.combo-name').text();
        ...
        if (quantity > 0) {
            ...
            var comboRow = '<tr class="tr_first">' +
                ...
                '<td class="td-second align-right" style="width: 110px;"><span class="sol">' + quantity + '</span> x ' + comboPrice.toLocaleString() + ' VNĐ</td>' +
                '</tr>';
    ...
    var totalRow = '<tr class="tr_second align-right">' +
        '<td colspan="2"><span class="total-text-">Tổng giá tiền</span></td>' +
        '</tr>' +
        '<tr class="align-right">' +
        '<td colspan="2"><span class="total-numb">' + totalMoney.toLocaleString() + '</span><span style="font-size: 24px; font-weight: bold;">VNĐ</span></td>' +
        '</tr>';
